...
@WebServlet({"/admin/mail-share"})
public class MailShareVideo extends HttpServlet{
...
//		SharesDao dao = new SharesDao();
		...
		if(req.getMethod().equalsIgnoreCase("post")) {
			if(req.getParameter("href") != null) {
				...
				separate(req, resp, video.getShares());
//				List<Shares> list = dao.findShareByTitle(video.getTitle());
...
//						req.setAttribute("list", separate(req, resp, list));
			...
			req.getRequestDispatcher("/view/admin/mail_share.jsp").forward(req, resp);
		}else req.getRequestDispatcher("/view/admin/mail_share.jsp").forward(req, resp);
		
	}
	
	private List<Shares> separate(HttpServletRequest req, HttpServletResponse resp, List<Shares> list)
		...
		if (surplus != 0)
			page++;
		List<Shares> result = new ArrayList<>();
		int input = 1;
		try {
